...
def render_chat_box(ticker):
    # [채팅창 렌더링 파트]
    # 1. 티커(ticker)를 이용해 해당 종목이 존재하는지 확인.
    # 2. 'stock_chats' 테이블에서 해당 종목에 달린 메시지 최신 20개를 긁어옴.
    # 3. JOIN을 써서 작성자의 닉네임까지 한 번에 가져옴
    # 4. 최신순으로 가져왔으니, 화면에 보여줄 때는 다시 시간순(reverse)으로 뒤집어서 전달
...
def create_chat():
    # 메시지 작성 파트
    # 1. 로그인이 안 된 유저는 401 에러
    # 2. 사용자가 보낸 티커와 메시지 내용이 비어있지 않은지 검사
    # 3. DB에 성공적으로 저장되면 새로고침 없이 화면을 업데이트할 수 있게 JSON(success)을 반환
