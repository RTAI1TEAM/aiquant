...
# [블루프린트 설정] 
# 이 코드를 'dashboard'라는 이름의 모듈로 등록해서 메인 서버(app.py)에서 불러다 쓸 수 있게 해줌
dashboard_bp = Blueprint("dashboard", __name__)


def get_yesterday_trade_summary(user_id):
    # [어제 거래 데이터 추출 헬퍼 함수]
    # 이 함수는 실제 DB에 접속해서 어제의 기록만 쏙쏙 골라오는 역할을 담당
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            # 1. 어제 발생한 개별 거래 리스트 조회 (최근 5개만)
            # CURDATE() - INTERVAL 1 DAY 를 써서 정확히 '어제' 날짜의 데이터만 가져옴
            cur.execute("""
                SELECT
                    t.id,
                    t.trade_type,
            ...
            # CASE 문을 써서 한 번의 쿼리로 매수/매도 각각의 개수를 효율적으로 계산
            cur.execute("""
                SELECT
