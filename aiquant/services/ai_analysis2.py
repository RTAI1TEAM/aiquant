"""
GEMINI API KEY 여러개를 돌려서 사용하여 API 호출 초과시 다른 KEY를 사용하여 API를 호출합니다.
"""
...
key_index = 0
last_called = [0] * len(gemini_clients)
MIN_INTERVAL = 13  # 같은 키 재사용 최소 간격
...
def get_gemini_client():
    ...
    while True:
        ...
        for _ in range(len(gemini_clients)):
            idx = key_index % len(gemini_clients)

            if now - last_called[idx] >= MIN_INTERVAL:
                last_called[idx] = now
                key_index = (idx + 1) % len(gemini_clients)
